 ...
 * import { dbConnect } from '@acessment/core-oes/database/mongoose.server';
 *
 ...
 */
export async function dbConnect(dbName: string, connectionString?: string): Promise<typeof mongoose> {
    ...
    if (!MONGODB_URI) {
        throw new Error("Please define MONGO_CONNECTION_STRING environment variable (server-side only)");
    }
    ...
    if (!cached.promise) {
        ...
        // Enable debug logging for MongoDB queries
